...
class Params:
	...
	def __init__(self,
		...
		self.kernel_concatenate = Kernel(kernel_func_concatenate, name, ['(rgb-of)', '(rgb-of-depth)'])
		self.C_mkl = C_mkl
	...
	def to_program_config(self):
		return ProgramConfig(name=self.name,
			...
			kernels=[self.kernel_rgb, self.kernel_of, self.kernel_depth],
			kernel_concatenate=self.kernel_concatenate,
			C_mkl=self.C_mkl,
			C_svms=[self.C_rgb, self.C_of, self.C_depth],
